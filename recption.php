...
<head>
    <style>
        body{
            background-image: url("resption.jpg");
...
<div class="container">
    ...
    <form class="form-horizontal"  action="recption.php" method="post">
...
<button type="button" class="btn btn-default"><a style="text-decoration: none" href='recption.php'>back</a></button>
...
{
    $stat1 = $GLOBALS['conn']->prepare("insert into Guest(id,guest_name,age)values (?,?,?)");
    $stat1->execute([$_POST['gid'], $_POST['gname'], $_POST['gage']]);
    $stat2 = $GLOBALS['conn']->prepare("insert into Reservation(guest_id,roomnumber,reservedtime)values (?,?,'CURRENT_TIMESTAMP')");
